    ...
    if (data.crew.at(key).name == crewMember) {
      ...
      myList.push(
        <li
          ...
          onClick={() => {
            setCrewMember(selectedMember.name);
          }}
        ></li>
      );
    } else {
      myList.push(
        <li
          key={data.crew.at(key).name}
          onClick={() => {
            setCrewMember(data.crew.at(key).name);
          }}
        ></li>
      );
    }
    ...
    <div className="crew-section">
      <div className="crew-details">
        ...
        <TransitionGroup component={crewMember.slice(0, 3)}>
          <CSSTransition timeout={1400} classNames="animation">
      ...
      <TransitionGroup component={crewMember.slice(0, 3)}>
        <CSSTransition timeout={1400} classNames="animation">
          <img src={require(`${selectedMember.images.webp}`)} alt="member" />
        </CSSTransition>
      </TransitionGroup>
    </div>
  );
