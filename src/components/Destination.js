    ...
    if (dest.toLocaleLowerCase() == planet.toLocaleLowerCase()) {
      planets.push(
        <li
          ...
          key={dest}
          onClick={() => {
            setPlanet(dest.toLocaleUpperCase());
          }}
        >
          {dest.toLocaleUpperCase()}
        </li>
      );
      ...
      <div className="view">
        <div className="planets">
          ...
          <TransitionGroup component={planet}>
            <CSSTransition timeout={1500} classNames="transition">
              <DestCard planet={selectedPlanetData} />
            </CSSTransition>
          </TransitionGroup>
        </div>
        <TransitionGroup component={planet[0].toLocaleLowerCase()}>
          <CSSTransition timeout={1500} classNames="animation">
            <img src={require(`${selectedPlanetData.images.webp}`)} />
          </CSSTransition>
        </TransitionGroup>
      </div>
    </div>
